...
(function(win, doc) {
    ...
    load("http://campaign.t99ltd.info/assets/lib/js/remplib.js");
})(window, document);
...
All examples use `http://campaign.t99ltd.info` as a base domain. Please change the host to the one you use
...
```shell
curl -X POST \
  http://campaign.t99ltd.info/api/banners/[BANNER_ID]/one-time-display \
...
$bannerId = 1;
$response = file_get_contents("http://campaign.t99ltd.info/api/banners/{$bannerId}/one-time-display", false, $context);
...
```shell
curl -X POST \
  http://campaign.t99ltd.info/api/segment-cache/provider/crm_segment/code/example_testing_segment/add-user \
  -H 'Accept: application/json' \
  -H 'Authorization: Bearer XXX' \
  -H 'Content-Type: application/json' \
  -d '{
...
$response = file_get_contents("http://campaign.t99ltd.info/api/segment-cache/provider/{$segmentProvider}/code/{$segmentCode}/add-user", false, $context);
// process response (raw JSON string)
```
...
Route `http://campaign.t99ltd.info/health` provides health check for database, Redis, storage and logging.
